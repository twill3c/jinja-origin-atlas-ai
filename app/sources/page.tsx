...
const SOURCES: Row[] = [
  ...
  },
  {
    ...
    use: "由緒テキストの意味解析(埋め込み・モチーフ・クラスタ)",
    license: "CC BY-SA 4.0",
    note: "本文と埋め込みベクトルは配布しない。配るのは数値スコアだけで、由来した記事の題名・版・ライセンスを神社ごとに表示する",
  },
  {
    name: "国土地理院",
  ...
  return (
    <main>
      ...
      <p style={{ fontSize: "0.9rem" }}>
        最後の点は宣言で終わらせず、機械で確かめている ——
        公開ファイルの中に長い文字列や長い数値列が入っていないかを走査し、
        入っていたらビルドを落とす。検査そのものが働くことも、
        わざと違反させた偽のファイルで確かめてある。
      </p>

      <h2>人が書いた表(祭神の画面で使う)</h2>
      <p style={{ fontSize: "0.9rem" }}>
        祭神の画面には、<strong>公開データから導いたのではない表</strong>が二つ入っている。
        どちらも神道史の通説にもとづいてこちらで起草し、人間が承認して凍結したもので、
        変更は専用のコミットで行う。出どころを混ぜないために、ここに分けて書いておく。
      </p>
      <ul style={{ fontSize: "0.9rem" }}>
        <li>
          <strong>総本社の表(26 柱)</strong> ——
          どの祭神のどこが総本社かを書いた表。リフトの答え合わせにだけ使い、
          <strong>リフトの計算には入れていない</strong>(入れれば一致は恒等式になる)。
          所在県は総本社の Wikidata Q-id から OSM のカタログを引き直して毎回確かめている。
          総本社が一意に定まらない祭神(菅原道真・素戔嗚尊・伊奘諾尊ほか)は理由を書いて除いた。
        </li>
        <li>
          <strong>祭神の対の分類(51 組)</strong> ——
          同じ社に並ぶ二柱の関係を、同一視 / 親族 / 総称と構成神 / その他の四つに分けた表。
          共起 9 回以上の対だけを分類し、<strong>それ未満は「未分類」と書いて空けてある</strong>。
          推測で型は付けない。
        </li>
      </ul>
    </main>
